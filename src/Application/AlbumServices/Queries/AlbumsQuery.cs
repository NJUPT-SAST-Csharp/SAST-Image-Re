...
public sealed record class AlbumsQuery(long? CategoryId, long? AuthorId, string? Title, Actor Actor)
    : IQueryRequest<List<AlbumDto>> { }

internal sealed class AlbumsQueryHandler(IQueryRepository<AlbumsQuery, List<AlbumDto>> repository)
