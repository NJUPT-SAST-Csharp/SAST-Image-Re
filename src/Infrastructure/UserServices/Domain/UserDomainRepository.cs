        ...
        IRepository<User, UserId>
{
    ...
    Task IRepository<User, Username>.DeleteAsync(Username id, CancellationToken cancellationToken)
    {
        throw new InvalidOperationException(
            "UserRepository using [Username] couldn't be used to complete the operation."
        );
    ...
    async Task IRepository<User, UserId>.DeleteAsync(UserId id, CancellationToken cancellationToken)
    {
        var user =
