...
internal sealed class AlbumDomainRepository(DomainDbContext context) : IRepository<Album, AlbumId>
{
    private readonly DomainDbContext _context = context;

    public async Task<AlbumId> AddAsync(Album entity, CancellationToken cancellationToken = default)
    {
    ...
    public Task<Album?> GetOrDefaultAsync(AlbumId id, CancellationToken cancellationToken = default)
    {
